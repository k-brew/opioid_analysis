...
```{r load.csv & packages}
library(pacman)
p_load(dplyr, readr, skimr, ggplot2, tidyverse, tidymodels, janitor)
...
## *Model 1*: Predicting Opioid Prescrption Rates based on non-opioid prescription rates?
...
```{r}
as.table(coef(fwd_step$finalModel,12))
```
```{r}
as.table(coef(back_step$finalModel,13))
```
